...
#' @param type Specifies which type of regression to run. Currently supports 'lm' (Default), 'lm_robust', 'lmer' and 'glm'. Has to correspond to the specified formula.
#' @param se_type Only applies to type = 'lm_robust'. Specifies the sort of (robust) SE sought. Allows "HC0", "HC1" (or "stata", the equivalent), "HC2", "HC3", or "classical". If clusters is specified the options are "CR0", "CR2", or "stata". See ?estimatr::lm_robust for more information.
#' @param clustervar Only applies to type = 'lm_robust'. Variable (in quotation marks) to apply for calculating clustered standard errors. See ?estimatr::lm_robust for more information.
...
mimargins <- function(df,
  ...
  if (type == "lm_robust") {
    ...
    ifelse(!require(estimatr),
           install.packages("estimatr", dep = T),
           require(estimatr))
    require(estimatr)
    ...
    if (ame == T) {
      if (is.null(at)) {
        ...
        test  <- lm_robust(formula = formula,
                           data = df[df[[imp_var]] == 1, ],
                           se_type = se_type,
                           clusters = df[df[[imp_var]] == 1, ][[paste0(clustervar)]])
        a <- summary(margins::margins(test,
                                      variables = variables))
        ...
        for (imp in unique(df[[imp_var]])) {
          reg <- lm_robust(formula = formula,
                    data = df[df[[imp_var]] == imp, ],
                    se_type = se_type,
                    clusters = df[df[[imp_var]] == imp, ][[paste0(clustervar)]])
    ...
    if (ame == F) {
      ...
      for (imp in unique(df[[imp_var]])) {
        reg <- summary(lm_robust(formula = formula,
                          data = df[df[[imp_var]] == imp, ],
                          se_type = se_type,
                          clusters = df[df[[imp_var]] == imp, ][[paste0(clustervar)]]))
